...
class Sample(_AbstractSample):
	...
	def __str__(self):
		return f"{type(self).__name__} {self.bname}"
...
class Dataset:
	...
	@classmethod
	def from_dir(cls, dir, mask_dir=None, *args, use_img_regex_for_masks=False, both_eyes_same_class=True, mirrored_offset=0, **kw):
		"""
		...
		:param bool use_img_regex_for_masks: If `True`, masks have the same basename as images and `mask_regex` will be ignored.
		...
		if mask_dir:
			data = cls._read_masked_samples(dir, mask_dir, use_img_regex_for_masks)
		else:
	...
	@classmethod
	def _read_masked_samples(cls, dir, mask_dir, use_img_regex):
		if not dir.is_dir():
			raise ValueError(f"{dir} is not a directory.")
		if not mask_dir.is_dir():
			raise ValueError(f"{mask_dir} is not a directory.")

		# Find valid image and mask files
		img_fs = {f.stem: f for f in cls._find_valid_files(dir, "Finding samples")}
		mask_fs = {f.stem: f for f in cls._find_valid_files(mask_dir, "Finding masks", not use_img_regex)}

		if not use_img_regex:
			# Parse the basenames to get basename and mask channel
			mask_matches = {f: cls.sample_cls.mask_match(f.stem) for f in mask_fs.values()}

			# Create dict of dicts, mapping: basename -> mask channel -> full file name
			mask_fs = {}
			for f, match in mask_matches.items():
	...
	def _find_valid_files(cls, dir, tqdm_text, mask=False):
		return [f for f in tqdm(dir.rglob('*'), desc=tqdm_text) if cls.sample_cls.valid(f, mask)]
