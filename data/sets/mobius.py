...
class MOBIUS(Dataset, sample_cls=MOBIUSSample):
	@classmethod
	def from_dir(cls, dir, mask_dir=True, *args, extra_info_path=Path('/data/MOBIUS.csv'), discard_bad=False, **kw):
		...
		if discard_bad:
			dataset = cls(s for s in dataset if s.n != 'bad')
		return dataset

	@classmethod
	def read_extra_info(cls, extra_info_path=Path('/data/MOBIUS.csv')):
