...
class Plot:
	...
	def interpolate(self, new_x=None, min_x=None, max_x=None, interp_points=5000, fill_value='extrapolate'):
		if new_x is None:
			new_x = np.linspace(self.x.min() if min_x is None else min_x, self.x.max() if max_x is None else max_x, interp_points)
		return Plot(new_x, sp.interpolate.interp1d(self.x, self.y, fill_value=fill_value)(new_x))


class PlotCollection(list):
	def average(self, interp_points=5000):
		new_x = np.linspace(min(p.x.min() for p in self), max(p.x.max() for p in self), interp_points)
		return Plot(new_x, np.vstack(p.interpolate(new_x).y for p in self).mean(axis=0))
...
class Painter(object):
	...
	def plot(self, plot, figure=None, figure_kw=None, **kw):
		"""
		Plot to a figure
